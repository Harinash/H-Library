            ...
            <div class="card-body">
              <table class="table table-bordered text-center" style="margin-top:30px;">
                ...
                <?php
                    ...
                    $sql = "SELECT borrow_tbl.UserID, reservation_tbl.ReservationID, reservation_tbl.BookID, book_tbl.Title, borrow_tbl.PenaltyAmount, borrow_tbl.DueDate, borrow_tbl.StatusReturn, borrow_tbl.PenaltyStatus, borrow_tbl.DatePaid 
                    FROM `borrow_tbl` 
                    ...
                    WHERE borrow_tbl.UserID = $user_id AND borrow_tbl.PenaltyStatus = 'Paid' AND borrow_tbl.PenaltyAmount > 0.00";
                    ...
                    if(mysqli_num_rows($query_no) >0){
                      foreach($query_no as $row){
                        ...
                        <td><?php echo $row['PenaltyAmount']?></td>
                        <td><?php echo htmlentities($row['DueDate'])?></td>
                        <td><?php echo $row['PenaltyStatus']?></td>
