        ...
        <li class="nav-item dropdown pe-3">
          ...
          <ul class="dropdown-menu dropdown-menu-end dropdown-menu-arrow profile">
            ...
            <h6><?php echo htmlentities($row_User['Username']) ?></h6> 
              <?php echo $row_User['UserRole'] ?></span> 
            </li>
            <li>
              <hr class="dropdown-divider">
            </li>
          ...
          <div class="card">
            ...
            <div class="card-body">
              <table class="table table-bordered text-center" style="margin-top:30px;">
                <thead>
                <tr style="background-color:#f4f2ef">
                    <th>Book Title</th>
                    <th>Reserved Date</th>
                ...
                <?php
                    ...
                    $sql = "SELECT reservation_tbl.ReservationDate, book_tbl.BookID, reservation_tbl.PickupDate, reservation_tbl.Status, reservation_tbl.UserID, reservation_tbl.CopyID, book_tbl.Title, reservation_tbl.ReservationID
                            FROM reservation_tbl
                            INNER JOIN user_tbl ON reservation_tbl.UserID = user_tbl.UserID
                            INNER JOIN book_tbl ON reservation_tbl.BookID = book_tbl.BookID
                            INNER JOIN bookcopy_tbl ON reservation_tbl.CopyID = bookcopy_tbl.CopyID
                            WHERE reservation_tbl.UserID = $user_id AND reservation_tbl.Status = 'Active'";
                    ...
                    if(mysqli_num_rows($query_no) >0){
                      foreach($query_no as $row){
                        ...
                        <td><a href="cancel_reservation.php?reservation_id=<?php echo $row['ReservationID'] ?>&user_id=<?php echo $user_id ?>&copy_id=<?php echo $row['CopyID'] ?>&cancel=<?php echo $cancel ?>" class="btn btn-danger" onclick="return confirmCancel()">Cancel</a></td>
                        <!-- <?php
                          if($row['UserID'] != $user_id){
                        ?>
                            <td><a href="cancel_reservation.php?id=<?php echo $row['UserID'] ?>" class="btn btn-danger">Delete</a></td>
                        <?php
                          }
                        ?> -->
                      </tr>
                      ...
                      ?>
                      <tr>
                        <td colspan="6">No current reservations</td>
