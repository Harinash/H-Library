...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $dislikeQuery = "SELECT * FROM review_likes WHERE ReviewID = $review_id AND UserID = $user_id";
    ...
    if (mysqli_num_rows($dislikeResult) > 0) {
        ...
        echo "AlreadyLiked"; // Indicate that the user has already disliked the post
    } else {
        // Check if the user has previously liked this post
        $likeQuery = "SELECT * FROM review_dislikes WHERE ReviewID = $review_id AND UserID = $user_id";
        ...
        if (mysqli_num_rows($likeResult) > 0) {
            ...
            $removeLikeQuery = "DELETE FROM review_dislikes WHERE ReviewID = $review_id AND UserID = $user_id";
            if (mysqli_query($dbconn, $removeLikeQuery)) {
                // Now, insert the dislike
                $insertDislikeQuery = "INSERT INTO review_likes (ReviewID, UserID) VALUES ($review_id, $user_id)";
